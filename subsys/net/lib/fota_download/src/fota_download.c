...
static uint32_t dl_host_sha256;
static uint32_t dl_file_sha256;
	...
	const enum dfu_target_image_type expected_type)
{
	...
	static uint32_t host_sha256;
	static uint32_t file_sha256;
	int err = -1;
	...
	host_sha256 = sys_hash32(host, strlen(host));
	file_sha256 = sys_hash32(file, strlen(file));
	LOG_DBG("#XFOTA: checsums %d,%d,%d,%d\r\n", host_sha256, file_sha256,
						    dl_host_sha256, dl_file_sha256);

	/* Verify if the URI is same as last time, if not, prevent resuming. */
	if (dl_host_sha256 != host_sha256 || dl_file_sha256 != file_sha256) {
		atomic_set_bit(&flags, FLAG_NEW_URI);
	} else {
		atomic_clear_bit(&flags, FLAG_NEW_URI);
	}
	dl_host_sha256 = host_sha256;
	dl_file_sha256 = file_sha256;
	dl_host = host;
	dl_file = file;
